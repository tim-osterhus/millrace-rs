...
- Added Rust release evidence for the Python `v0.18.6..v0.19.0` parity pass,
  covering execution capability contracts/config, compiled capability grants,
  pre-dispatch capability gates, durable approval storage, approval CLI and
  mailbox routing, runner support/evidence metadata, run-inspection capability
  output, docs, package verification, and the optional `millrace-web`
  `v0.19.0` package/version unsupported-gap surface.
- Added release fixture coverage for v0.19.0 parity fixtures, runtime docs,
  source package mapping, required Builder verification command results,
  package verification, generated-cache package exclusions, and forbidden
  release actions.
...
- Bumped the Rust crate, lockfile package metadata, and version-visible CLI
  output from `0.3.5` to `0.4.0`.
- Updated README, roadmap, source-package map, runtime docs, outline, fixture
  docs, and release evidence to target Python `v0.19.0` at
  `efb9c5881f524d23dcb78aecfc96fdf7cda9d26f`.

### Known Gaps

- The optional Python `packages/millrace-web` `v0.19.0` package version,
  `millrace-ai>=0.19.0` dependency floor, and FastAPI app version are recorded
