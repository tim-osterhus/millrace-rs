...
`v0.17.4..v0.18.0` contract into the `millrace-ai` `0.3.0` package.
...
| Compiler, mode, graph, thinking-level materialization, and compiled-stage-graph exports | `src/compiler/` | `tests/compiler_contracts.rs`, `tests/compiler_materialization.rs`, `tests/compiler_parity.rs` |
| Runtime tick, daemon, closure, governance, learning, run-trace persistence, and monitor behavior | `src/runtime/` | `tests/runtime_serial.rs`, `tests/runtime_daemon.rs` |
...
version and runtime dependency, and Python `v0.18.0` added compiled graph
exports, run-trace summaries, recent-trace Flow overlays, trace outcome labels,
package version/dependency sync, and read-only/no-lock evidence to that web
surface. The Rust crate records those surfaces in
`tests/fixtures/cli_parity/web_dashboard_parity_decision.json` and
`tests/fixtures/cli_parity/auto_port_v0_18_0_release_parity_evidence.json` as
...
The `0.3.0` release evidence is tied to Python `v0.17.4..v0.18.0`, target tag
`v0.18.0`, commit `e4ccf099c8345a8b8708cdaa1ac510bdc7851387`. The final
