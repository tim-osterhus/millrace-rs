...
`v0.18.1..v0.18.2` contract into the `millrace-ai` `0.3.2` package.
...
| Compiler, mode, graph, thinking-level materialization, compiled-stage-graph exports, Recon graph assets, and opt-in Integrator graph assets | `src/compiler/` | `tests/compiler_contracts.rs`, `tests/compiler_materialization.rs`, `tests/compiler_parity.rs` |
| Runtime tick, daemon, closure, governance, learning, Recon result application/hardening, Integrator routing, stage/work-item ownership, run-trace persistence, and monitor behavior | `src/runtime/` | `tests/runtime_serial.rs`, `tests/runtime_daemon.rs` |
...
and Python `v0.18.1` synced the optional package version, `millrace-ai>=0.18.1`
dependency floor, and FastAPI application version, and Python `v0.18.2` repeats
that package/runtime version sync at `0.18.2`. The Rust crate records those
surfaces in
`tests/fixtures/cli_parity/web_dashboard_parity_decision.json` and
`tests/fixtures/cli_parity/auto_port_v0_18_2_release_parity_evidence.json` as
...
The `0.3.2` release evidence is tied to Python `v0.18.1..v0.18.2`, target tag
`v0.18.2`, commit `5444cb9485ea90b67b2ed6ba7e0723ae9fe7b79f`. The
