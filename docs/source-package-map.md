...
`v0.18.3..v0.18.4` contract into the `millrace-ai` `0.3.4` package.
...
| Runtime tick, daemon, closure, governance, learning, Recon result application/hardening, Integrator routing, stage/work-item ownership, Planner-to-Librarian triggers, blocked metadata persistence, daemon stranded-dependency recovery, run-trace persistence, and monitor behavior | `src/runtime/` | `tests/runtime_serial.rs`, `tests/runtime_daemon.rs` |
| Runner request/result artifacts, Codex CLI, Pi RPC, fake runner, active work item source metadata normalization, and blocked recovery failure classification | `src/runners/` | `tests/runners_codex_cli.rs`, `tests/runners_pi_rpc.rs`, `tests/runners_normalization.rs`, `tests/runners_live_smoke.rs` |
| Workspace layout, queue/state stores, managed assets, lifecycle integrity, locks, and audited blocked-task requeue transitions | `src/workspace/`, `src/workspace.rs` | `tests/workspace_assets_baseline.rs`, `tests/workspace_queue_state_stores.rs`, `tests/workspace_doctor.rs` |
...
and Python `v0.18.1`, `v0.18.2`, `v0.18.3`, and `v0.18.4` repeat
that package/runtime version sync through `0.18.4`. The Rust crate records those
surfaces in
`tests/fixtures/cli_parity/web_dashboard_parity_decision.json` and
`tests/fixtures/cli_parity/auto_port_v0_18_4_release_parity_evidence.json` as
...
The `0.3.4` release evidence is tied to Python `v0.18.3..v0.18.4`, target tag
`v0.18.4`, commit `516e947e90155b6436dbc9efcf932254f34bc39c`. The
