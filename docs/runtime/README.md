...
Python runtime docs changed through `v0.18.0`; they do not claim production status or
introduce a Rust web dashboard.

Start with:

- `millrace-cli-reference.md`: version output plus graph/trace CLI inspection
  commands
...
`millrace-agents/` workspace artifact format. Python `v0.18.0` remains the
reference for the release delta represented by Rust `0.3.0`.
