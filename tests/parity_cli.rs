...
use millrace_ai::contracts::{
    ActiveRunRequestKind, ActiveRunState, ClosureTargetState, Plane, StageName, Timestamp,
    WorkItemKind,
};
...
fn rust_version_command_has_millrace_shape() {
    ...
    assert_eq!(version_line.version, "0.2.0");
...
fn committed_slice8_advanced_parity_evidence_covers_all_surfaces_and_live_tests() {
    ...
    let allowed_rust_refs = BTreeSet::from([
        ...
        "tests/runtime_serial.rs::serial_tick_learning_trigger_enqueues_targeted_curator_request",
...
fn rust_crate_release_metadata_and_package_include_rules_are_0_2_0() {
    ...
    for forbidden in ["millrace-agents/**", "ideas/**", "target/**", "README.md"] {
        assert!(
            !include
                .iter()
    ...
    assert!(
        ...
        "Cargo.lock package metadata must track the 0.2.0 crate version"
    );
}
...
fn rust_compile_show_renders_representative_inspection_fields() {
    ...
    for expected in [
        ...
        "learning_triggers: 3\n",
        "learning_trigger: execution.doublechecker.success-to-curator\n",
...
fn rust_queue_intake_routes_to_mailbox_when_daemon_lock_is_active() {
    ...
    let envelope: Value =
        serde_json::from_str(&fs::read_to_string(&mailbox_paths[0]).unwrap()).unwrap();
    assert_eq!(envelope["kind"], "mailbox_command");
    assert_eq!(envelope["command"], "add_task");
    assert_eq!(
        envelope["payload"]["document"]["task_id"],
        "task-mailbox-cli"
    );
}
